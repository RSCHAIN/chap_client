...
function Maritime() {
    ...
    const [arriv, setArriv] = useState("Côte D'Ivoire");
    ...
    const [choixCarton, setchoixCarton] = useState(["Carton 200L"]);
    const [valeurEnEuro, setValeurEnEuro] = useState([0]);
    ...
    const handleAddGroup = (e) => {
        ...
        ///reforme du visuel ajustement des fonctions 

        description.push("")
        choixCarton.push("Carton 200L")
        valeurEnEuro.push(0)
        besoin.push(false)
    ...
    const handleRemoveGroup = (groupId) => {
        ...
        const descriptionUpdated = description.filter((_, index) => index !== groupId);
        const choixCartonUpdated = choixCarton.filter((_, index) => index !== groupId);
        const valeurEnEuroUpdated = valeurEnEuro.filter((_, index) => index !== groupId);
        const besoinUpdated = besoin.filter((_, index) => index !== groupId);

        setDescription(descriptionUpdated);
        setchoixCarton(choixCartonUpdated);
        setValeurEnEuro(valeurEnEuroUpdated);
        setBesoin(besoinUpdated);
    ...
    const handleSaveDataInSessionStorage = (arriv, destination,email, poste, rue, ville, lieuReceptionColis, description, choixCarton, valeurEnEuro) => {
        sessionStorage.setItem("arrivee", JSON.stringify(arriv));
        ...
        sessionStorage.setItem("addresse", JSON.stringify(rue));
        sessionStorage.setItem("ville", JSON.stringify(ville));
        sessionStorage.setItem("receptioncolis", JSON.stringify(lieuReceptionColis));
        sessionStorage.setItem("descriptioncolis", JSON.stringify(description));
        sessionStorage.setItem("choixcarton", JSON.stringify(choixCarton));
        sessionStorage.setItem("valeureneuro", JSON.stringify(valeurEnEuro));
        sessionStorage.setItem("besoin", JSON.stringify(besoin));
                                ...
                                {inputGroups.map((group, groupId) => (
                                    <div key={groupId} className="mb-6">
                                        <div className="flex flex-col lg:flex-row justify-between gap-4 lg:gap-0">
                                            <textarea name="" id="" onChange={(e) => description[groupId] = e.target.value} placeholder="Description ici..." cols="30" rows="3" className="sm:text-sm py-2 pl-4 border border-slate-800"></textarea>
                                        ...
                                        <div className="mt-6">
                                            <input name="materiel" id="materiel" type="checkbox" onChange={(e)=>besoin[groupId]=e.target.checked} value={besoin[groupId]}/>
                                            <label htmlFor="materiel" className="px-4 text-red-800">Besoin du materiel ?</label>
                                        </div>
